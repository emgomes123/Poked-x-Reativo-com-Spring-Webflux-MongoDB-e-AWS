package com.emgomes.pokedex.controller;

import com.emgomes.pokedex.model.Pokemon;
import com.emgomes.pokedex.model.PokemonEvent;
import com.emgomes.pokedex.repository.PokedexRepository;
...
public class PokemonController {
    ...
    @PutMapping("{id}")
    public Mono<ResponseEntity<Pokemon>> updadePokemon(@PathVariable(value = "id")
                                                       String id,
        ...
        return repository.findById(id)
            ...
            return repository.save(existingPokemon);
                ...
                .map(updadePokemon -> ResponseEntity.ok(updadePokemon))
                        .defaultIfEmpty(ResponseEntity.notFound().build());
    ...
    public Flux<PokemonEvent> getPokemonEvents(){
        return Flux.interval(Duration.ofSeconds(5))
                .map(val ->
                        new PokemonEvent(val,"Evento de Pokemoooon"));
    }
